...
func newRecruitHandler(studyRepo *db.StudyRepository, recruitRepo *db.RecruitRepository, reactionHandler *ReactionHandler) func(ctx context.Context, s *discordgo.Session, i *discordgo.InteractionCreate) {
	return func(ctx context.Context, s *discordgo.Session, i *discordgo.InteractionCreate) {
		...
		logCommand(ctx, i, "start", "recruit requested branch=%s channel=%s from=%s to=%s", branch, channelID, fromStr, toStr)
		...
		if err != nil {
			respondError(ctx, s, i, fmt.Sprintf("Invalid 'from' date format. Use YYYY-MM-DD. (got: %s)", fromStr))
			return
		}
		toDate, err := time.Parse("2006-01-02", toStr)
		if err != nil {
			respondError(ctx, s, i, fmt.Sprintf("Invalid 'to' date format. Use YYYY-MM-DD. (got: %s)", toStr))
			return
		}
		if !toDate.After(fromDate) {
			respondError(ctx, s, i, "'to' date must be after 'from' date.")
			return
		}
		...
		for idx := range studies {
			if err := s.MessageReactionAdd(channelID, msg.ID, numberEmojis[idx], discordgo.WithContext(ctx)); err != nil {
				logCommand(ctx, i, "error", "failed to add reaction emoji=%s message=%s err=%v", numberEmojis[idx], msg.ID, err)
			}
		}
		...
		logCommand(ctx, i, "success", "recruit posted branch=%s studies=%d message=%s channel=%s", branch, len(studies), msg.ID, channelID)
	}
}
